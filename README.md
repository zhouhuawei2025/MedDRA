...
5. 后端逐条或批量生成待编码术语向量
6. 在对应版本的 Qdrant collection 中检索 Top 10 候选
7. 若结果达到高置信阈值，则直接输出候选结果
8. 若结果置信度不足，则将原始术语 + Top 10 候选交给 LLM 做重排序
9. 后端返回 `最优 / 次优 / 较优` 和是否调用 AI
10. 用户确认后下载结果 Excel
...
1. 先做向量检索，获取 Top 10 候选
2. 若满足高置信条件，则直接返回结果
3. 若不满足，则调用 LLM 进行候选重排序
